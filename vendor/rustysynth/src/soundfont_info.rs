    ...
    pub(crate) fn new<R: Read>(reader: &mut R) -> Result<Self, SoundFontError> {
        ...
        while reader.bytes_read() < end {
            ...
            match id.as_bytes() {
                ...
                // Skip unknown INFO sub-chunks (including nested LIST chunks such
                // as the "xdta" list some tools embed), rather than rejecting them.
                _ => BinaryReader::discard_data(reader, size)?,
            }
        }
