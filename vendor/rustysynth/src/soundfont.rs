    ...
    fn sanity_check(&mut self) -> Result<(), SoundFontError> {
        ...
        for instrument in &mut self.instruments {
            for region in &mut instrument.regions {
                ...
                // These are unrecoverable: the oscillator would read out of
                // bounds or produce a zero/negative-length sample.
                if start < 0 || end as usize >= self.wave_data.len() || end <= start {
                    return Err(SoundFontError::SanityCheckFailed);
                }

                // Degenerate or out-of-range loop points are recoverable. Some
                // fonts (e.g. ACCURATE_SF2_AiX_CTX800) encode "no loop" by
                // setting start_loop >= end_loop (often == start) while still
                // marking the region as looping. Forcing NoLoop plays the sample
                // straight through instead of stalling on a zero-length loop.
